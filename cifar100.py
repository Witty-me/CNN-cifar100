...
def inference(images):
  ...
  with tf.variable_scope('conv1_') as scope:
    ...
    biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
    pre_activation = tf.nn.bias_add(conv, biases)
    conv1_ = tf.nn.relu(pre_activation, name=scope.name)
  ...
  norm1_ = tf.nn.lrn(conv1_, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                   name='norm1_')
  # dropout1
  drop1_ = tf.nn.dropout(pool1, 0.9);
  ...
  with tf.variable_scope('conv2') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         ...
                                         stddev=5e-2,
                                         wd=0.0)
    conv = tf.nn.conv2d(drop1_, kernel, [1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
    pre_activation = tf.nn.bias_add(conv, biases)
...
# dropout4
  ...
  with tf.variable_scope('local3') as scope:
    ...
    reshape = tf.reshape(drop4, [FLAGS.batch_size, -1])
