...
def _activation_summary(x):
    ...
    tensor_name = re.sub('%s_[0-99]*/' % TOWER_NAME, '', x.op.name)
...
def inference(images):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        conv1 = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
    ...
    with tf.variable_scope('conv1_') as scope:
        ...
        biases = _variable_on_cpu('biases', [100], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv1_ = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
        _activation_summary(conv1_)

    # norm1_
    norm1_ = tf.nn.lrn(conv1_, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                       name='norm1_')
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(norm1_, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [200], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv2 = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
    ...
    norm2 = tf.nn.lrn(pool2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                      name='norm2')
    drop2 = tf.nn.dropout(norm2, 0.9)

    # conv2_ : NiN layer
    with tf.variable_scope('conv2_') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[1, 1, 200, 200],
                                             stddev=5e-2,
                                             wd=0.0)
        ...
        biases = _variable_on_cpu('biases', [200], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv2_ = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
        _activation_summary(conv2_)

    # norm2_
    norm2_ = tf.nn.lrn(conv2_, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                       name='norm2_')
    ...
    with tf.variable_scope('conv3') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(norm2_, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [300], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv3 = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
    ...
    norm3 = tf.nn.lrn(pool3, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                      name='norm3')
    drop3 = tf.nn.dropout(norm3, 0.9)

    # conv3_ : NiN layer
    with tf.variable_scope('conv3_') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[1, 1, 300, 300],
                                             stddev=5e-2,
                                             wd=0.0)
        ...
        biases = _variable_on_cpu('biases', [300], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv3_ = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
        _activation_summary(conv3_)

    # norm3_
    norm3_ = tf.nn.lrn(conv3_, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                       name='norm3_')
    ...
    with tf.variable_scope('conv4') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(norm3_, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [400], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv4 = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
    ...
    # conv4_ : NiN layer
    with tf.variable_scope('conv4_') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[1, 1, 400, 400],
                                             stddev=5e-2,
                                             wd=0.0)
        ...
        biases = _variable_on_cpu('biases', [400], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv4_ = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
        _activation_summary(conv4_)

    # norm4_
    norm4_ = tf.nn.lrn(conv4_, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                       name='norm4_')
    ...
    with tf.variable_scope('conv5') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             ...
                                             stddev=5e-2,
                                             wd=0.0)
        conv = tf.nn.conv2d(norm4_, kernel, [1, 1, 1, 1], padding='SAME')
        biases = _variable_on_cpu('biases', [500], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv5 = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
    ...
    # conv5_ : NiN layer
    with tf.variable_scope('conv5_') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[1, 1, 500, 500],
                                             stddev=5e-2,
                                             wd=0.0)
        ...
        biases = _variable_on_cpu('biases', [500], tf.constant_initializer(0.1))
        pre_activation = tf.nn.bias_add(conv, biases)
        conv5_ = tf.nn.leaky_relu(pre_activation, alpha=0.3, name=scope.name)
        _activation_summary(conv5_)

    # norm5_
    norm5_ = tf.nn.lrn(conv5_, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                       name='norm5_')

    # local3
    with tf.variable_scope('local3') as scope:
        # Move everything into depth so we can perform a single matrix multiply.
        reshape = tf.reshape(norm5_, [FLAGS.batch_size, -1])
